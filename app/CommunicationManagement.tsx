...
function CommunicationModal({ teams, item, onClose, onSaved }: { teams: TeamRecord[]; item: Communication | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="communication-modal" onMouseDown={(event) => event.stopPropagation()}><header><div><span className="eyebrow">NOVO AVISO</span><h2>{item ? "Editar comunicado" : "Criar comunicado"}</h2><p>O sistema salvará a lista exata de destinatários.</p></div><button className="modal-close" onClick={onClose}><X size={18} strokeWidth={1.75} /></button></header><form onSubmit={submit}>
