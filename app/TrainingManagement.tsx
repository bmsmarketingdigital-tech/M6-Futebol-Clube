...
export function TrainingManagement({ teams, notify }: { teams: TeamRecord[]; notify: (message: string) => void }) {
  ...
  return <>
    <div className="section-heading"><div><span className="eyebrow">METODOLOGIA BASEFORTE</span><h1>Treinos</h1><p>Planeje sessões completas e mantenha o histórico por turma.</p></div><button className="primary-button" onClick={() => setModal("new")}>＋ Planejar treino</button></div>
    <section className="training-metrics">
      <article className="metric-card"><div className="metric-icon green">◫</div><div><span>PLANEJADOS</span><strong>{planned.length}</strong><small>próximas sessões</small></div></article>
      <article className="metric-card"><div className="metric-icon blue">✓</div><div><span>CONCLUÍDOS</span><strong>{completed.length}</strong><small>no histórico</small></div></article>
      <article className="metric-card"><div className="metric-icon orange">◷</div><div><span>VOLUME TOTAL</span><strong>{minutes} min</strong><small>planejamento registrado</small></div></article>
    </section>
    <section className="training-board">
      {trainings.map((training) => <article className="card training-session-card" key={training.id}>
        ...
        <div className="session-meta"><span>◷ {training.durationMinutes} min</span><span>◎ {training.drills.length} exercícios</span></div>
      ...
      {!loading && trainings.length === 0 && <div className="card class-empty"><span>◫</span><strong>Nenhum treino planejado</strong><small>Crie uma sessão e organize os exercícios em sequência.</small><button className="primary-button" onClick={() => setModal("new")}>Planejar primeiro treino</button></div>}
    </section>
...
function TrainingModal({ teams, training, onClose, onSaved }: { teams: TeamRecord[]; training: Training | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="training-modal" onMouseDown={(event) => event.stopPropagation()}>
    <header><div><span className="eyebrow">PLANEJAMENTO DE SESSÃO</span><h2>{training ? "Editar treino" : "Novo treino"}</h2><p>Organize objetivo, duração e sequência metodológica.</p></div><button className="modal-close" onClick={onClose}>×</button></header>
